...
const useBoundingBox2 = (dependencies = []) => {
  ...
  useEffect(() => {
    ...
    setBoundingBox(
      ref.current?.querySelector('.row__poster').getBoundingClientRect().width
    );
  ...
  useEffect(() => {
    ...
    const recalculate = debounce(() => {
      if (ref.current) {
        setBoundingBox(
          ref.current?.querySelector('.row__poster').getBoundingClientRect()
            .width
        );
      }
    }, 250);
...
export default useBoundingBox2;
